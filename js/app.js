/* ════════════════════════════
   app.js  v6.2 — แก้ไข: CSS vars, badge classes, ICON→FA, getCurrentUser
════════════════════════════ */
...
/* ── ICON — FA icons ── */
const ICON = {
  pos:'<i class="fa-solid fa-store"></i>',
  dash:'<i class="fa-solid fa-chart-line"></i>',
  hist:'<i class="fa-solid fa-receipt"></i>',
  products:'<i class="fa-solid fa-box-open"></i>',
  stock:'<i class="fa-solid fa-warehouse"></i>',
  withdraw:'<i class="fa-solid fa-boxes-packing"></i>',
  stocklog:'<i class="fa-solid fa-clipboard-list"></i>',
  customers:'<i class="fa-solid fa-users"></i>',
  accounts:'<i class="fa-solid fa-gear"></i>',
  report:'<i class="fa-solid fa-chart-bar"></i>',
  promotions:'<i class="fa-solid fa-gift"></i>',
  loginlog:'<i class="fa-solid fa-lock"></i>',
  cash:'<i class="fa-solid fa-money-bill-wave"></i>',
  transfer:'<i class="fa-solid fa-mobile-screen-button"></i>',
  profit:'<i class="fa-solid fa-chart-line"></i>',
  cost:'<i class="fa-solid fa-coins"></i>',
  ...
  export:'<i class="fa-solid fa-file-csv"></i>',
  ...
  pay:'<i class="fa-solid fa-credit-card"></i>',
  scan:'<i class="fa-solid fa-barcode"></i>',
  filter:'<i class="fa-solid fa-magnifying-glass"></i>',
  calendar:'<i class="fa-solid fa-calendar-days"></i>',
  brand:'<i class="fa-solid fa-store"></i>',
};
...
function showSplash() {
  ...
  splash.innerHTML = `
    ...
    <div class="splash-ring"></div><div class="splash-ring"></div><div class="splash-ring"></div>
...
function spawnSplashParticles(container) {
  ...
  for (let i=0;i<28;i++) {
    const p=document.createElement('div'),angle=(i/28)*Math.PI*2,dist=120+Math.random()*180,size=4+Math.random()*8;
    const tx=`translateX(${Math.cos(angle)*dist}px) translateY(${Math.sin(angle)*dist}px)`;
    p.className='splash-particle';
    p.style.cssText=`width:${size}px;height:${size}px;background:${colors[i%colors.length]};left:${cx}px;top:${cy}px;--tx:${tx};animation-duration:${0.6+Math.random()*0.4}s;animation-delay:${i*0.015}s;`;
...
function applyLangToUI() {
  ...
  const navMap = {pos:'pos',dash:'dash',hist:'hist',products:'products',stock:'stock',withdraw:'withdraw',stocklog:'stocklog',customers:'customers',accounts:'accounts',report:'report',promotions:'promotions',loginlog:'loginlog'};
...
function initPullToRefresh() {
  ...
  main.addEventListener('touchend',   e=>{ if(!pulling)return; pulling=false; const ind=$('pull-indicator');if(ind)ind.style.display='none'; if(e.changedTouches[0].clientY-startY>80){showToast('รีเฟรชแล้ว','ok');navigate(getCurrentPage());} });
}
...
function launchConfetti() {
  const colors=['#52B788','#74C69D','#FF6B35','#2D6A4F','#FFB08A','#F4A300','#FFFFFF'];
...
function flyToCart(srcEl) {
  if (!srcEl) return;
  const src=srcEl.getBoundingClientRect(),ct=document.querySelector('.cart-title');if(!ct)return;
  const tgt=ct.getBoundingClientRect(),dot=document.createElement('div');
  dot.style.cssText=`position:fixed;z-index:9999;pointer-events:none;width:20px;height:20px;border-radius:50%;background:linear-gradient(135deg,var(--g-700),var(--g-500));left:${src.left+src.width/2-10}px;top:${src.top+src.height/2-10}px;transition:all .55s cubic-bezier(.4,.05,.3,1);font-size:12px;display:flex;align-items:center;justify-content:center;color:#fff`;
  dot.innerHTML='<i class="fa-solid fa-cart-shopping" style="font-size:9px"></i>';
...
function initApp() {
  ...
  if (u.avatarImg) {
    ...
    avatarEl.style.background=isOwner?'linear-gradient(135deg,var(--g-700),var(--g-500))':'linear-gradient(135deg,var(--g-500),var(--p-400))';
  }
  ...
  applyLangToUI();
  DB.accounts.forEach(a=>{if(!a.avatar||a.avatar.startsWith('fa-'))return;a.avatar=a.role==='owner'?'fa-crown':a.role==='manager'?'fa-user-tie':'fa-user';});
  ...
  setTimeout(()=>{
    if(typeof initFirebase==='function'){
      ...
      if(ok){
        setupPresence(u.id); listenPresence(); listenFirebase();
        if(u.role==='owner'){
          _fbDb.ref('pos/_lastPush').once('value',snap=>{
            if(!snap.val()){showToast('กำลังอัปโหลดข้อมูลครั้งแรก...','ok');pushAllToFirebase();}
          });
        }
      }
    }
...
function toggleLang() { STATE.reportLang=STATE.reportLang==='th'?'en':'th'; applyLangToUI(); navigate(getCurrentPage()); }
...
function updateOnlineBanner(){
  ...
  const byBranch={};online.forEach(a=>{if(!byBranch[a.branch])byBranch[a.branch]=[];byBranch[a.branch].push(a.name);});
  banner.style.display='flex';
  banner.innerHTML=online.length
    ?`<i class="fa-solid fa-circle" style="color:var(--g-500);font-size:8px"></i><strong>สาขาออนไลน์:</strong>&nbsp;`+Object.entries(byBranch).map(([b,names])=>`${b} (${names.join(', ')})`).join(' • ')+`<button onclick="this.parentElement.style.display='none'" style="margin-left:auto;background:none;border:none;cursor:pointer;font-size:14px;color:inherit"><i class="fa-solid fa-xmark"></i></button>`
    :`<i class="fa-solid fa-circle" style="color:var(--txt-muted);font-size:8px"></i>ไม่มีสาขาออนไลน์ขณะนี้<button onclick="this.parentElement.style.display='none'" style="margin-left:auto;background:none;border:none;cursor:pointer;font-size:14px;color:inherit"><i class="fa-solid fa-xmark"></i></button>`;
}
...
function checkLowStockAlert(){
  ...
  if(low.length){el.style.display='flex';el.innerHTML=`<i class="fa-solid fa-triangle-exclamation"></i> สต็อกใกล้หมด: <strong style="margin-left:4px">${low.map(p=>p.name).join(', ')}</strong><button onclick="this.parentElement.style.display='none'" style="margin-left:auto;background:none;border:none;cursor:pointer;font-size:16px;color:var(--warning)"><i class="fa-solid fa-xmark"></i></button>`;}
...
function renderPOS(){
  ...
  $('pos-cats').innerHTML=`<div class="cat-tab active" onclick="setPosCategory('all',this)"><i class="fa-solid fa-border-all"></i> ทั้งหมด</div>`+cats.map(c=>`<div class="cat-tab" onclick="setPosCategory('${c}',this)">${catIcon(c)} ${c}</div>`).join('');
...
function applyPosFilter(){
  ...
  if(!list.length){el.innerHTML=`<div class="empty-state"><i class="fa-solid fa-magnifying-glass" style="font-size:40px;color:var(--g-200);display:block;margin-bottom:12px"></i><p>ไม่พบสินค้า</p></div>`;return;}
...
function buildProductCard(p,uid,isOwner,isManage){
  ...
  /* ── badge สต็อก — ใช้ class ที่ถูกต้องตาม style.css ── */
  const stockBadge=out
    ?`<div class="pc-stock-badge badge-out"><i class="fa-solid fa-xmark"></i> หมด</div>`
    :low?`<div class="pc-stock-badge badge-low"><i class="fa-solid fa-triangle-exclamation"></i> เหลือ ${stk}</div>`
    :'';

  const manageBtn=isManage
    ?`<div class="pc-manage-btns">
        <button class="btn btn-secondary btn-xs" onclick="openEditProduct(${p.id})" style="flex:1"><i class="fa-solid fa-pen"></i> ${t('edit')}</button>
        <button class="btn btn-secondary btn-xs" onclick="openReviewModal(${p.id})" title="รีวิว"><i class="fa-solid fa-star"></i></button>
        <button class="btn btn-danger btn-xs btn-icon" onclick="deleteProduct(${p.id})" title="${t('delete')}"><i class="fa-solid fa-trash-can"></i></button>
       </div>`:'';

  const ratingRow=avg
    ?`<div class="product-card-rating"><i class="fa-solid fa-star" style="color:var(--warning)"></i> ${avg} <span style="color:var(--txt-faint);font-size:9px">(${revs.length})</span></div>`
    :(isManage?`<div class="product-card-rating" style="color:var(--txt-faint)">${t('noReview')}</div>`:'');

  return `<div class="product-card${out?' out-of-stock':''}" id="pc-${p.id}" ${!isManage?`onclick="addToCart(${p.id},event)"`:'style="cursor:default"'}>
    ...
    <div class="product-card-body">
      ...
      <div class="product-card-stock ${out?'out':low?'low':''}">
        ${out?'<i class="fa-solid fa-xmark"></i>':low?'<i class="fa-solid fa-triangle-exclamation"></i>':'<i class="fa-solid fa-check"></i>'}
        ${out?t('outOfStock'):low?`${t('lowStock')} ${stk}`:`${t('inStock')} ${stk}`}
      </div>
      ${ratingRow}
      ${manageBtn}
    </div>
...
function renderPromoBar(){
  ...
  el.innerHTML=`<span style="font-weight:800;margin-right:8px"><i class="fa-solid fa-gift"></i> ${t('promo')}:</span>`+
    promos.map(p=>`<span class="promo-chip${STATE.selectedPromoId===p.id?' active':''}" onclick="selectPromo(${p.id})">${p.name}</span>`).join('')+
    (STATE.selectedPromoId?`<span class="promo-chip active" onclick="selectPromo(null)"><i class="fa-solid fa-xmark"></i> ยกเลิก</span>`:'');
}
...
function addToCart(pid,event){
  ...
  if(ci){if(ci.qty<stk)ci.qty++;else{showToast('สต็อกไม่พอ','warn');return;}}
...
function renderCart(){
  ...
  if(!STATE.cart.length){
    el.innerHTML=`<div class="cart-empty"><i class="fa-solid fa-cart-shopping" style="font-size:34px;opacity:.3;display:block;margin-bottom:10px;color:var(--g-500)"></i><p>ยังไม่มีสินค้า</p></div>`;
  ...
  el.innerHTML=STATE.cart.map((ci,i)=>`
    ...
    (discount>0?`<div style="display:flex;justify-content:space-between;padding:8px 0;font-size:12px;color:var(--danger);font-weight:700;border-top:1px dashed rgba(224,60,82,.3)"><span><i class="fa-solid fa-tag"></i> ${t('discount')}</span><span>-฿${fmt(discount)}</span></div>`:'');
...
function changeCartQty(i,delta){
  ...
  else if(ci.qty>stk){ci.qty=stk;showToast('สต็อกไม่พอ','warn');}
...
function openPayment(){
  ...
  refreshPaymentSummary(); selectPayMethod('cash');
...
function searchPayMember(q){
  ...
  if(found){STATE.selectedCustomerId=found.id;renderCartCustomer();renderPayMemberResult(found);refreshPaymentSummary();}
  else renderPayMemberResult('notfound');
}
...
function renderPayMemberResult(cust,total){
  ...
  if(cust==='notfound'){
    el.innerHTML=`<div style="display:flex;align-items:center;gap:8px;padding:10px 12px;background:var(--danger-bg);border:1px solid rgba(224,62,82,.2);border-radius:var(--r-sm);font-size:12px;color:var(--danger)">
      <i class="fa-solid fa-user-slash"></i> ไม่พบสมาชิก — ตรวจสอบเบอร์อีกครั้ง</div>`;return;
  }
  ...
  el.innerHTML=`<div style="display:flex;align-items:center;gap:12px;padding:12px 14px;background:var(--g-50);border:1.5px solid var(--g-200);border-radius:var(--r-md);position:relative;overflow:hidden">
    ...
    <div style="width:38px;height:38px;border-radius:10px;background:linear-gradient(135deg,var(--g-700),var(--g-500));display:flex;align-items:center;justify-content:center;flex-shrink:0">
      <i class="fa-solid ${tierIcon}" style="color:#fff;font-size:16px"></i>
    </div>
    <div style="flex:1;min-width:0">
      <div style="font-family:'Mitr',sans-serif;font-weight:700;font-size:14px;color:var(--txt-primary)">${cust.name}</div>
      <div style="font-size:10px;color:var(--txt-muted);margin-top:1px"><i class="fa-solid fa-phone" style="font-size:9px"></i> ${cust.phone||'—'} · มา ${cust.visitCount||0} ครั้ง</div>
    </div>
...
function calcChange(){
  ...
  if(recv>0){box.style.display='block';$('change-value').textContent='฿'+fmt(Math.max(0,chg));box.style.borderColor=chg>=0?'rgba(82,183,136,.3)':'rgba(224,60,82,.3)';$('change-value').style.color=chg>=0?'var(--g-700)':'var(--danger)';}
...
function confirmPayment(){
  ...
  launchConfetti();showToast(`ชำระ ฿${fmt(total)} สำเร็จ!`,'ok',4000);applyPosFilter();checkLowStockAlert();
...
function voidSale(saleId){
  ...
  showConfirm(`ยืนยันยกเลิกบิล #${saleId}?\nสต็อกจะถูกคืนกลับ`,()=>{
    ...
    sale.items.forEach(ci=>{const p=DB.products.find(x=>x.id===ci.id);if(!p)return;if(isOwnerSale){const b=p.centralStock;p.centralStock+=ci.qty;addStockLog(p.id,'add',ci.qty,b,p.centralStock,`ยกเลิกบิล #${saleId}`);}else{if(!p.branchStock[uid])p.branchStock[uid]=0;const b=p.branchStock[uid];p.branchStock[uid]+=ci.qty;addStockLog(p.id,'add',ci.qty,b,p.branchStock[uid],`ยกเลิกบิล #${saleId}`);}});
...
function printReceipt(saleId){
  ...
  const bars=Array.from(barSeed+String(sale.total)).map((c,i)=>{const n=(c.charCodeAt(0)*17+i*7)%3;return `<rect x="${i*6}" y="0" width="${n+1}" height="28" fill="#1B4332"/>`;}).join('');
...
<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Sarabun',sans-serif;background:#f5f0e8;display:flex;align-items:flex-start;justify-content:center;min-height:100vh;padding:20px}.receipt{background:#fff;width:320px;border-radius:16px;overflow:hidden;box-shadow:0 20px 60px rgba(61,35,20,.18)}.rhead{background:linear-gradient(135deg,#1B4332 0%,#2D6A4F 50%,#52B788 100%);padding:24px 22px 20px;position:relative;overflow:hidden}.brand-name{font-family:'Prompt',sans-serif;font-size:18px;font-weight:800;color:#fff}.brand-sub{font-size:10px;color:rgba(255,255,255,.7);margin-top:2px}.bill-meta{margin-top:14px;padding-top:12px;border-top:1px solid rgba(255,255,255,.18);display:flex;justify-content:space-between}.bill-no strong{font-family:'Prompt',sans-serif;font-size:22px;font-weight:800;color:#fff;display:block;line-height:1}.bill-date{text-align:right;font-size:10px;color:rgba(255,255,255,.65)}.items-section{padding:16px 18px 0}.section-label{font-size:9px;font-weight:700;color:#a0623a;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:10px;display:flex;align-items:center;gap:8px}.section-label::after{content:'';flex:1;height:1px;background:#e8ceb0}.item-row{display:flex;align-items:flex-start;gap:8px;padding:7px 0;border-bottom:1px dashed #f0e4d0}.item-row:last-child{border:none}.item-name{flex:1;font-size:13px;color:#3d2314;font-weight:500}.item-qty{font-size:11px;color:#a0623a;margin-top:2px}.item-price{font-family:'Prompt',sans-serif;font-weight:700;font-size:14px;color:#2D6A4F;flex-shrink:0}.summary{padding:12px 18px;background:#fafaf8;margin-top:10px}.sum-row{display:flex;justify-content:space-between;padding:4px 0;font-size:12px;color:#6b3a20}.sum-row.total-row{border-top:2px solid #2D6A4F;margin-top:8px;padding-top:10px;font-family:'Prompt',sans-serif}.total-val{font-size:26px;font-weight:900;color:#1B4332;line-height:1}.zigzag{height:12px;background:#fff;position:relative;overflow:hidden}.zigzag::before{content:'';display:block;height:100%;background:radial-gradient(circle at 6px 100%,#f5f0e8 6px,transparent 7px);background-size:12px 12px;background-repeat:repeat-x;background-position:0 0}.zigzag.top::before{background:radial-gradient(circle at 6px 0%,#f5f0e8 6px,transparent 7px);background-size:12px 12px;background-repeat:repeat-x}.loyalty-section{margin:0 18px 12px;padding:12px 14px;background:linear-gradient(135deg,#f0faf2,#fff);border:1.5px solid #b7e4c7;border-radius:10px}.loyalty-points{font-family:'Prompt',sans-serif;font-size:18px;font-weight:900;color:#2D6A4F}.barcode-section{padding:14px 18px;text-align:center}.barcode-num{font-size:10px;color:#c8a880;letter-spacing:2px;font-family:monospace}.rfoot{background:linear-gradient(135deg,#f0faf2,#fef9f5);padding:16px 18px 20px;text-align:center;border-top:1px dashed #d4a27a}.thanks{font-family:'Prompt',sans-serif;font-size:14px;font-weight:700;color:#1B4332;margin-bottom:4px}.tagline{font-size:10px;color:#a0623a;letter-spacing:1px}@media print{body{background:none;padding:0}.receipt{box-shadow:none;border-radius:0;width:100%}.print-btn{display:none!important}}</style>
</head><body><div class="receipt">
${sale.voided?'<div style="background:#fef2f2;border-left:4px solid #e03e52;padding:10px 16px;font-size:12px;font-weight:700;color:#b91c1c">บิลนี้ถูกยกเลิกแล้ว</div>':''}
<div class="rhead">
  <div style="display:flex;align-items:center;gap:12px">
    <div style="width:44px;height:44px;border-radius:10px;background:rgba(255,255,255,.2);border:1.5px solid rgba(255,255,255,.3);display:flex;align-items:center;justify-content:center;font-size:20px">🏪</div>
    <div><div class="brand-name">PUNKANOMKEAW</div><div class="brand-sub">${sale.branch||'ร้านค้า'}</div></div>
  </div>
  <div class="bill-meta">
    <div class="bill-no"><span style="font-size:11px;color:rgba(255,255,255,.65)">หมายเลขบิล</span><strong>#${String(sale.id).padStart(4,'0')}</strong></div>
    <div class="bill-date">${dateStr}<br><span style="font-weight:600;color:rgba(255,255,255,.85)">${sale.seller}</span></div>
  </div>
</div>
<div class="zigzag top"></div>
<div class="items-section">
  <div class="section-label">รายการสินค้า</div>
  ${sale.items.map((it,i)=>`<div class="item-row"><div style="width:20px;height:20px;border-radius:5px;background:#f0faf2;color:#2D6A4F;font-size:10px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0">${i+1}</div><div style="flex:1"><div class="item-name">${it.name}</div><div class="item-qty">${fmt(it.price)} × ${it.qty}</div></div><div class="item-price">฿${fmt(it.price*it.qty)}</div></div>`).join('')}
</div>
<div class="summary">
  ${sale.discount>0?`<div class="sum-row"><span>ส่วนลด ${promo?.name?'('+promo.name+')':''}</span><span style="color:#e03e52">-฿${fmt(sale.discount)}</span></div>`:''}
  <div class="sum-row total-row"><div><div style="font-size:13px;font-weight:700;color:#1B4332">ยอดรวม</div><div style="display:inline-flex;align-items:center;gap:5px;background:#f0faf2;border:1px solid #b7e4c7;border-radius:20px;padding:4px 10px;font-size:11px;font-weight:700;color:#2D6A4F;margin-top:6px">${methodText}</div></div><div class="total-val">฿${fmt(sale.total)}</div></div>
</div>
${cust?`<div class="zigzag"></div><div class="loyalty-section"><div style="display:flex;justify-content:space-between;align-items:center"><div style="font-size:13px;font-weight:700;color:#1B4332">👤 ${cust.name}</div><div style="text-align:right"><div class="loyalty-points">${fmt(cust.points)} แต้ม</div><div style="font-size:10px;color:#52B788;font-weight:600">+${earned} แต้มจากบิลนี้</div></div></div></div>`:''}
<div class="zigzag"></div>
<div class="barcode-section">
  <svg display="block" style="margin:0 auto 5px" width="${barSeed.length*6+12}" height="36" viewBox="0 0 ${barSeed.length*6+12} 36"><g transform="translate(6,4)">${bars}</g></svg>
  <div class="barcode-num">${String(sale.id).padStart(4,'0')} ${new Date(sale.time).getFullYear()} ${String(sale.total).padStart(6,'0')}</div>
</div>
<div class="rfoot"><div class="thanks">ขอบคุณที่ใช้บริการ</div><div class="tagline">PUNKANOMKEAW · ${sale.branch||''}</div></div>
<div style="text-align:center;padding:10px;background:#f5f0e8">
  <button class="print-btn" onclick="window.print()" style="background:linear-gradient(135deg,#1B4332,#52B788);color:#fff;border:none;padding:10px 28px;border-radius:20px;font-family:'Prompt',sans-serif;font-weight:700;font-size:13px;cursor:pointer">พิมพ์ใบเสร็จ</button>
</div>
</div><script>window.onload=function(){document.fonts.ready.then(()=>{setTimeout(()=>window.print(),300);})};<\/script></body></html>`);
  w.document.close();
}
...
async function openBarcodeScanner(){
  ...
  try{_scannerStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:'environment'}});video.srcObject=_scannerStream;video.play();scanFrame(video);}
...
async function scanFrame(video){
  ...
  if('BarcodeDetector' in window){
    ...
    const loop=async()=>{if(!_scannerStream)return;try{const bs=await det.detect(video);if(bs.length>0){const code=bs[0].rawValue;closeBarcodeScanner();const p=DB.products.find(x=>x.sku&&x.sku.toLowerCase()===code.toLowerCase());if(p){addToCart(p.id);showToast(`สแกน: ${p.name}`,'ok');}else{$('pos-search-input').value=code;onPosSearch(code);}return;}}catch(e){}requestAnimationFrame(loop);};loop();
...
function getFilteredSales(){
  const u=getCurrentUser();if(!u)return[];
  const now=new Date();let s=DB.sales.filter(x=>!x.voided);
  ...
  return s.filter(x=>{const d=new Date(x.time);if(STATE.dashPeriod==='day')return d.toDateString()===now.toDateString();if(STATE.dashPeriod==='week'){const w=new Date(now);w.setDate(w.getDate()-7);return d>=w;}if(STATE.dashPeriod==='month')return d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear();return d.getFullYear()===now.getFullYear();});
}
...
function renderDashboard(){
  ...
  if(bsel&&isOwner){
    ...
    bsel.innerHTML=`<button class="period-tab ${STATE.dashBranch==='all'?'active':''}" onclick="setDashBranch('all')">${t('allBranch')}</button>`+brs.map(b=>`<button class="period-tab ${STATE.dashBranch==b.id?'active':''}" onclick="setDashBranch(${b.id})">${b.branch}</button>`).join('');
  }else if(bsel)bsel.style.display='none';

  const statsEl=$('dash-stats');
  if(isOwner){
    statsEl.innerHTML=`
      <div class="stat-card c-green"><div class="stat-icon si-green"><i class="fa-solid fa-coins"></i></div><div class="stat-label">${t('total')}</div><div class="stat-value v-green" id="sc-total">฿0</div></div>
      <div class="stat-card c-teal"><div class="stat-icon si-teal"><i class="fa-solid fa-chart-line"></i></div><div class="stat-label">${t('netProfit')}</div><div class="stat-value v-teal" id="sc-profit">฿0</div><div style="font-size:10px;color:var(--txt-muted);margin-top:2px" id="sc-margin"></div></div>
      <div class="stat-card c-blue"><div class="stat-icon si-blue"><i class="fa-solid fa-receipt"></i></div><div class="stat-label">${t('bills')}</div><div class="stat-value v-blue" id="sc-bills">0</div></div>
      <div class="stat-card c-orange"><div class="stat-icon si-orange"><i class="fa-solid fa-calculator"></i></div><div class="stat-label">${t('avg')}</div><div class="stat-value v-orange" id="sc-avg">฿0</div></div>`;
    setTimeout(()=>{
    ...
    statsEl.innerHTML=`
      <div class="stat-card c-blue"><div class="stat-icon si-blue"><i class="fa-solid fa-receipt"></i></div><div class="stat-label">${t('bills')}</div><div class="stat-value v-blue" id="sc-bills">0</div></div>
      <div class="stat-card c-teal"><div class="stat-icon si-teal"><i class="fa-solid fa-box-open"></i></div><div class="stat-label">${t('items')}</div><div class="stat-value v-teal" id="sc-items">0</div></div>`;
  ...
  if(cc){
    ...
    if(ctx&&window.Chart){
      ...
      _chartInst=new Chart(ctx,{type:'line',data:{labels,datasets:[{label:t('total'),data:dvals,borderColor:'#2D6A4F',backgroundColor:'rgba(82,183,136,0.08)',borderWidth:2.5,pointBackgroundColor:'#2D6A4F',pointRadius:4,tension:0.4,fill:true},...(isOwner?[{label:t('profit'),data:pvals,borderColor:'#F4A300',backgroundColor:'rgba(244,163,0,0.06)',borderWidth:2,pointBackgroundColor:'#F4A300',pointRadius:3,tension:0.4,fill:false}]:[])]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:isOwner,position:'top',labels:{font:{size:11},boxWidth:12}},tooltip:{callbacks:{label:c=>`${c.dataset.label}: ฿${fmt(c.parsed.y)}`}}},scales:{x:{grid:{display:false},ticks:{font:{size:11}}},y:{grid:{color:'rgba(0,0,0,0.06)'},ticks:{callback:v=>'฿'+fmt(v),font:{size:11}}}}}});
    }
  }

  const pm={};DB.sales.filter(s=>!s.voided).forEach(s=>s.items.forEach(it=>{pm[it.name]=(pm[it.name]||0)+it.qty*it.price;}));
  const tops=Object.entries(pm).sort((a,b)=>b[1]-a[1]).slice(0,5);
  $('dash-top-products').innerHTML=tops.length
    ?tops.map(([name,val],i)=>`<div class="mini-list-item"><div class="mini-rank">${['🥇','🥈','🥉','4','5'][i]}</div><div class="mini-name">${name}</div>${isOwner?`<div class="mini-val">฿${fmt(val)}</div>`:'<div class="mini-val" style="color:var(--txt-muted)">ขายดี</div>'}</div>`).join('')
  ...
  $('dash-pay-breakdown').innerHTML=isOwner?`<div style="display:flex;flex-direction:column;gap:12px;margin-top:6px">
    <div><div class="flex-row mb-1"><span style="font-size:12px"><i class="fa-solid fa-money-bill-wave"></i> ${t('cash')}</span><div class="spacer"></div><span class="fw-bold text-green">฿${fmt(cash)}</span></div><div style="background:rgba(82,183,136,.1);border-radius:5px;height:7px;overflow:hidden"><div style="background:var(--g-500);height:100%;border-radius:5px;width:${total?Math.round(cash/total*100):0}%;transition:width .8s ease"></div></div></div>
    <div><div class="flex-row mb-1"><span style="font-size:12px"><i class="fa-solid fa-mobile-screen-button"></i> ${t('transfer')}</span><div class="spacer"></div><span class="fw-bold" style="color:var(--info)">฿${fmt(tran)}</span></div><div style="background:rgba(58,124,165,.1);border-radius:5px;height:7px;overflow:hidden"><div style="background:var(--info);height:100%;border-radius:5px;width:${total?Math.round(tran/total*100):0}%;transition:width .8s ease"></div></div></div>
    <div class="flex-row" style="background:rgba(224,60,82,.06);border-radius:8px;padding:8px 10px"><span style="font-size:12px"><i class="fa-solid fa-coins"></i> ${t('cost')}</span><div class="spacer"></div><span class="fw-bold" style="color:var(--danger)">฿${fmt(cost)}</span></div>
    <div class="flex-row" style="background:rgba(82,183,136,.06);border-radius:8px;padding:8px 10px"><span style="font-size:12px"><i class="fa-solid fa-chart-line"></i> ${t('netProfit')}</span><div class="spacer"></div><span class="fw-bold text-green">฿${fmt(profit)}</span></div>
    <button class="btn btn-secondary btn-sm" onclick="exportSalesCSV()" style="width:100%;justify-content:center;margin-top:4px"><i class="fa-solid fa-file-csv"></i> ${t('export')}</button>
    </div>`:`<div class="empty-state" style="padding:20px"><p><i class="fa-solid fa-lock"></i> เฉพาะเจ้าของร้าน</p></div>`;
  ...
  $('dash-low-stock').innerHTML=low.length
    ...
    :`<div class="empty-state" style="padding:20px"><p><i class="fa-solid fa-circle-check" style="color:var(--g-500)"></i> สต็อกปกติ</p></div>`;
}
...
function renderReport(){
  ...
  if(!u||!canSeeFinance(u.role)){$('page-report').innerHTML=`<div class="denied-state"><i class="fa-solid fa-lock" style="font-size:46px;color:var(--g-200);margin-bottom:16px;display:block"></i><h3>เฉพาะเจ้าของร้าน</h3></div>`;return;}
  const now=new Date();let sales=DB.sales.filter(s=>!s.voided);
  ...
  const bySeller={};sales.forEach(s=>{if(!bySeller[s.seller])bySeller[s.seller]={name:s.seller,branch:s.branch,total:0,profit:0,bills:0};bySeller[s.seller].total+=s.total;bySeller[s.seller].profit+=s.profit||0;bySeller[s.seller].bills++;});
  const sellerRows=Object.values(bySeller).sort((a,b)=>b.total-a.total);

  const byProd={};sales.forEach(s=>s.items.forEach(i=>{if(!byProd[i.name])byProd[i.name]={name:i.name,qty:0,revenue:0,cost:0};byProd[i.name].qty+=i.qty;byProd[i.name].revenue+=i.price*i.qty;byProd[i.name].cost+=(i.costPrice||0)*i.qty;}));
  const prodRows=Object.values(byProd).sort((a,b)=>b.revenue-a.revenue).slice(0,10);

  const el=$('report-content');if(!el)return;
  el.innerHTML=`
  <div class="grid-4 mb-3">
    <div class="stat-card c-green"><div class="stat-icon si-green"><i class="fa-solid fa-coins"></i></div><div class="stat-label">${t('total')}</div><div class="stat-value v-green">฿${fmt(total)}</div></div>
    <div class="stat-card c-teal"><div class="stat-icon si-teal"><i class="fa-solid fa-chart-line"></i></div><div class="stat-label">${t('netProfit')}</div><div class="stat-value v-teal">฿${fmt(profit)}</div><div style="font-size:10px;color:var(--txt-muted);margin-top:2px">${t('margin')} ${margin}%</div></div>
    <div class="stat-card c-orange"><div class="stat-icon si-orange"><i class="fa-solid fa-coins"></i></div><div class="stat-label">${t('cost')}</div><div class="stat-value v-orange">฿${fmt(cost)}</div></div>
    <div class="stat-card c-blue"><div class="stat-icon si-blue"><i class="fa-solid fa-receipt"></i></div><div class="stat-label">${t('bills')}</div><div class="stat-value v-blue">${sales.length}</div></div>
  </div>
  <div class="grid-2 mb-3">
    <div class="glass-card">
      <div class="card-header"><span class="card-header-title"><i class="fa-solid fa-user"></i> ${t('total')} — ${t('seller')}</span></div>
      <div class="glass-card-pad" style="padding:12px 16px">
        ${sellerRows.map(s=>`<div class="mini-list-item"><div style="flex:1"><div style="font-size:13px;font-weight:700">${s.name}</div><div style="font-size:10px;color:var(--txt-muted)">${s.branch} • ${s.bills} ${t('bills')}</div></div><div style="text-align:right"><div class="mini-val">฿${fmt(s.total)}</div><div style="font-size:10px;color:var(--g-500)">${t('profit')} ฿${fmt(s.profit)}</div></div></div>`).join('')||`<div class="empty-state" style="padding:16px"><p>ไม่มีข้อมูล</p></div>`}
      </div>
    </div>
    <div class="glass-card">
      <div class="card-header"><span class="card-header-title"><i class="fa-solid fa-box-open"></i> ${t('items')} Top 10</span></div>
      <div class="glass-card-pad" style="padding:12px 16px">
        ${prodRows.map((p,i)=>`<div class="mini-list-item"><div class="mini-rank">${i+1}</div><div style="flex:1"><div style="font-size:12px;font-weight:700">${p.name}</div><div style="font-size:10px;color:var(--txt-muted)">${p.qty} ชิ้น</div></div><div style="text-align:right"><div class="mini-val">฿${fmt(p.revenue)}</div><div style="font-size:10px;color:var(--g-500)">${t('profit')} ฿${fmt(p.revenue-p.cost)}</div></div></div>`).join('')||`<div class="empty-state" style="padding:16px"><p>ไม่มีข้อมูล</p></div>`}
      </div>
    </div>
  </div>
  <div class="flex-row mb-2" style="gap:10px">
    <button class="btn btn-primary" onclick="exportSalesCSV()"><i class="fa-solid fa-file-csv"></i> ${t('export')}</button>
    <button class="btn btn-secondary" onclick="printReport()"><i class="fa-solid fa-print"></i> พิมพ์รายงาน</button>
  </div>`;
}
...
function printReport(){
  ...
  const total=sales.reduce((s,x)=>s+x.total,0),profit=sales.reduce((s,x)=>s+(x.profit||0),0),cost=sales.reduce((s,x)=>s+(x.cost||0),0);
  const w=window.open('','_blank','width=800,height=600');
  const periodLabel={day:'วันนี้',week:'7 วันที่ผ่านมา',month:'เดือนนี้',year:'ปีนี้'}[_reportPeriod];
  w.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><style>body{font-family:'Sarabun',sans-serif;padding:32px;font-size:13px;color:#222}h1{font-size:22px;font-weight:900;color:#2D6A4F;margin-bottom:4px}.sub{color:#888;font-size:12px;margin-bottom:20px}table{width:100%;border-collapse:collapse;margin-top:16px}th{background:#e8f8ef;padding:8px 12px;text-align:left;font-size:11px;font-weight:800;color:#3a6b4e;text-transform:uppercase}td{padding:8px 12px;border-top:1px solid #e0ece6;font-size:12px}.summary{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px}.sum-card{background:#f0faf4;border:1px solid #aaebc6;border-radius:8px;padding:12px 16px}.sum-label{font-size:10px;color:#7aab8c;font-weight:800;text-transform:uppercase}.sum-val{font-size:20px;font-weight:900;color:#2D6A4F;margin-top:4px}</style></head><body>
    ...
    <tbody>${sales.slice(0,100).map(s=>`<tr><td>${s.id}</td><td>${fmtDate(s.time)}</td><td>${s.seller}</td><td>${s.items.map(i=>`${i.name}×${i.qty}`).join(', ')}</td><td>฿${fmt(s.total)}</td><td>฿${fmt(s.cost||0)}</td><td>฿${fmt(s.profit||0)}</td><td>${s.method==='cash'?'เงินสด':'โอน'}</td></tr>`).join('')}</tbody></table>
...
function renderHistory(q){
  ...
  setTimeout(()=>{
    if(!list.length){el.innerHTML=`<div class="empty-state"><i class="fa-solid fa-receipt" style="font-size:40px;color:var(--g-200);display:block;margin-bottom:12px"></i><p>ไม่พบรายการ</p></div>`;return;}
    ...
    const rows=list.slice(0,200).map(s=>{
      ...
      return `<tr class="${s.voided?'voided-row':''}">
        <td style="color:var(--txt-faint);font-size:11px">${s.id}</td>
        <td>${s.items.map(i=>`${i.name} ×${i.qty}`).join(', ')}${s.voided?`<br><span class="tag tag-danger" style="font-size:9px">ยกเลิก</span>`:''}${promo?`<br><span class="tag tag-warning" style="font-size:9px"><i class="fa-solid fa-gift"></i> ${promo.name}</span>`:''}</td>
        <td><span class="tag ${s.method==='cash'?'tag-success':'tag-info'}">${s.method==='cash'?'<i class="fa-solid fa-money-bill-wave"></i>':'<i class="fa-solid fa-mobile-screen-button"></i>'}</span></td>
        <td style="font-size:12px;color:var(--txt-muted)">${s.seller}</td>
        ${cust?`<td style="font-size:11px;color:var(--g-700)"><i class="fa-solid fa-user"></i> ${cust.name}</td>`:'<td style="color:var(--txt-faint)">—</td>'}
        <td style="font-size:11px;color:var(--txt-muted);white-space:nowrap">${fmtDate(s.time)}</td>
        ${isOwner
          ?`<td class="fw-bold font-prompt ${s.voided?'text-muted':'text-green'}">${s.voided?'—':'฿'+fmt(s.total)}</td>
             <td style="font-size:11px;color:var(--g-500)">${s.voided?'—':'฿'+fmt(s.profit||0)}</td>`
          :'<td>—</td><td>—</td>'}
        <td>
          <div style="display:flex;gap:3px">
            <button class="btn btn-secondary btn-xs" onclick="openCommentModal(${s.id})"><i class="fa-solid fa-comment-dots"></i>${cmt?' '+cmt:''}</button>
            <button class="btn btn-secondary btn-xs" onclick="printReceipt(${s.id})"><i class="fa-solid fa-print"></i></button>
            ${!s.voided&&(isOwner||s.sellerId===uid)?`<button class="btn btn-danger btn-xs" onclick="voidSale(${s.id})"><i class="fa-solid fa-ban"></i></button>`:''}
          </div>
        </td>
      </tr>`;
    }).join('');
    el.innerHTML=stats+
      `<div class="flex-row mb-2" style="flex-wrap:wrap;gap:8px">
        <div style="display:flex;align-items:center;gap:6px;font-size:12px;color:var(--txt-muted)">
        ...
        ${isOwner?`<button class="btn btn-secondary btn-sm" onclick="exportSalesCSV()" style="margin-left:auto"><i class="fa-solid fa-file-csv"></i> CSV</button>`:''}
      </div>
...
function openCommentModal(saleId){
  ...
  $('comment-modal-body').innerHTML=`
    <div style="font-size:12px;color:var(--txt-muted);margin-bottom:12px">บิล #${saleId} — ${sale?sale.items.map(i=>i.name).join(', '):''}</div>
    <div style="margin-bottom:14px">${cmts.length?cmts.map(c=>`<div class="comment-item"><div class="comment-meta">${c.userName} • ${fmtDate(c.time)}</div><div class="comment-text">${c.text}</div></div>`).join(''):'<div style="font-size:12px;color:var(--txt-faint);text-align:center;padding:10px">ยังไม่มีคอมเมนต์</div>'}</div>
...
function renderPromotions(){
  const u=getCurrentUser();
  if(!u||!canManageProducts(u.role)){$('page-promotions').innerHTML=`<div class="denied-state"><i class="fa-solid fa-lock" style="font-size:46px;color:var(--g-200);margin-bottom:16px;display:block"></i><h3>เฉพาะเจ้าของร้าน</h3></div>`;return;}
  const el=$('promotions-content');if(!el)return;
  if(!DB.promotions.length){el.innerHTML=`<div class="empty-state"><i class="fa-solid fa-gift" style="font-size:40px;color:var(--g-200);display:block;margin-bottom:12px"></i><p>ยังไม่มีโปรโมชั่น</p></div>`;return;}
  el.innerHTML=`<div class="glass-card">${DB.promotions.map(p=>`
    <div class="acc-row">
      <div style="font-size:28px;width:44px;text-align:center;color:var(--g-700)">${p.type==='percent'?'<i class="fa-solid fa-percent"></i>':'<i class="fa-solid fa-baht-sign"></i>'}</div>
      <div style="flex:1">
        <div class="fw-bold" style="font-size:14px">${p.name}</div>
        <div style="font-size:11px;color:var(--txt-muted)">${promoDesc(p)} • ใช้แล้ว ${p.usedCount||0} ครั้ง</div>
      </div>
      ...
      <button class="btn btn-danger btn-xs" onclick="deletePromo(${p.id})"><i class="fa-solid fa-trash-can"></i></button>
    </div>`).join('')}</div>`;
}
function promoDesc(p){
  ...
  if(p.endDate){const now=new Date(),end=new Date(p.endDate),diff=Math.ceil((end-now)/(1000*60*60*24));if(diff<0)base+=' <span style="color:var(--danger);font-weight:700">· หมดอายุแล้ว</span>';else if(diff===0)base+=' <span style="color:var(--danger);font-weight:700">· หมดคืนนี้</span>';else if(diff<=3)base+=` <span style="color:var(--warning);font-weight:700">· เหลือ ${diff} วัน</span>`;else base+=` · ถึง ${new Date(p.endDate).toLocaleDateString('th-TH',{day:'numeric',month:'short'})}`;}
  return base;
}
function openAddPromo(){openModal('modal-promo');$('promo-name').value='';$('promo-value').value='';$('promo-minspend').value='';$('promo-type').value='percent';if($('promo-start'))$('promo-start').value='';if($('promo-end'))$('promo-end').value='';togglePromoType();}
function togglePromoType(){const type=$('promo-type').value;$('promo-value-group').style.display=type!=='minspend'?'block':'none';$('promo-minspend-group').style.display=type==='minspend'?'block':'none';$('promo-discount-group').style.display=type==='minspend'?'block':'none';}
function savePromo(){
  const name=$('promo-name').value.trim(),type=$('promo-type').value;
  if(!name){showToast('กรอกชื่อโปรฯ','err');return;}
  const startVal=$('promo-start')?.value,endVal=$('promo-end')?.value;
  if(endVal&&startVal&&new Date(endVal)<new Date(startVal)){showToast('วันสิ้นสุดต้องหลังวันเริ่มต้น','err');return;}
  DB.promotions.push({id:getNextPromoId(),name,type,active:true,usedCount:0,value:type!=='minspend'?parseFloat($('promo-value').value)||0:0,minSpend:type==='minspend'?parseFloat($('promo-minspend').value)||0:0,discount:type==='minspend'?parseFloat($('promo-discount').value)||0:0,startDate:startVal?new Date(startVal):null,endDate:endVal?new Date(endVal+'T23:59:59'):null});
  closeModal('modal-promo');saveDB();showToast('เพิ่มโปรโมชั่นแล้ว','ok');renderPromotions();
}
function promoIsExpired(p){if(!p.endDate)return false;return new Date(p.endDate)<new Date();}
function togglePromo(id){const p=DB.promotions.find(x=>x.id===id);if(p){p.active=!p.active;saveDB();renderPromotions();showToast(p.active?'เปิดโปรฯ แล้ว':'ปิดโปรฯ แล้ว','ok');}}
function deletePromo(id){showConfirm('ลบโปรโมชั่นนี้?',()=>{DB.promotions=DB.promotions.filter(p=>p.id!==id);saveDB();showToast('ลบแล้ว','ok');renderPromotions();});}

/* ══════════════ PRODUCTS ══════════════ */
function renderProducts(q){
  const u=getCurrentUser();if(!u)return;
  if(!canManageProducts(u.role)){$('page-products').innerHTML=`<div class="denied-state"><i class="fa-solid fa-lock" style="font-size:46px;color:var(--g-200);margin-bottom:16px;display:block"></i><h3>ไม่มีสิทธิ์เข้าถึง</h3><p>เฉพาะเจ้าของร้านเท่านั้น</p></div>`;return;}
  $('product-grid-mgmt').innerHTML=skeletonGrid(6);
  setTimeout(()=>{const list=q?DB.products.filter(p=>p.name.toLowerCase().includes(q.toLowerCase())):DB.products;$('product-grid-mgmt').innerHTML=list.map(p=>buildProductCard(p,u.id,true,true)).join('');},200);
}
function openAddProduct(){const u=getCurrentUser();if(!canManageProducts(u?.role)){showToast('เฉพาะเจ้าของร้าน','err');return;}STATE.editProductId=null;STATE.tempImg=null;$('modal-prod-title').innerHTML='<i class="fa-solid fa-plus"></i> เพิ่มสินค้าใหม่';['fp-name','fp-cat','fp-sku','fp-cost'].forEach(id=>{const e=$(id);if(e)e.value='';});['fp-price','fp-stock'].forEach(id=>$(id).value='');$('img-dropzone').innerHTML='<i class="fa-solid fa-cloud-arrow-up"></i><span>คลิกเพื่อเลือกรูป</span>';openModal('modal-product');}
function openEditProduct(id){const u=getCurrentUser();if(!canManageProducts(u?.role)){showToast('เฉพาะเจ้าของร้าน','err');return;}const p=DB.products.find(x=>x.id===id);STATE.editProductId=id;STATE.tempImg=p.img;$('modal-prod-title').innerHTML='<i class="fa-solid fa-pen"></i> แก้ไขสินค้า';$('fp-name').value=p.name;$('fp-cat').value=p.cat;$('fp-price').value=p.price;$('fp-cost').value=p.costPrice||0;$('fp-stock').value=p.centralStock;$('fp-sku').value=p.sku||'';$('img-dropzone').innerHTML=p.img?`<img src="${p.img}">`:'<i class="fa-solid fa-cloud-arrow-up"></i><span>คลิกเพื่อเลือกรูป</span>';openModal('modal-product');}
...
function saveProduct(){
  const name=$('fp-name').value.trim(),cat=$('fp-cat').value.trim()||'อื่นๆ',price=parseFloat($('fp-price').value),costPrice=parseFloat($('fp-cost').value)||0,stock=parseInt($('fp-stock').value),sku=$('fp-sku').value.trim();
  if(!name||isNaN(price)||isNaN(stock)){showToast('กรอกข้อมูลให้ครบ','err');return;}
  if(STATE.editProductId){const p=DB.products.find(x=>x.id===STATE.editProductId);Object.assign(p,{name,cat,price,costPrice,centralStock:stock,sku,img:STATE.tempImg});}
  else DB.products.push({id:getNextProductId(),name,cat,price,costPrice,sku,img:STATE.tempImg,centralStock:stock,branchStock:{},reviews:[]});
  closeModal('modal-product');saveDB();showToast(STATE.editProductId?'แก้ไขสำเร็จ':'เพิ่มสำเร็จ','ok');renderProducts();
}
function deleteProduct(id){const u=getCurrentUser();if(!canManageProducts(u?.role)){showToast('เฉพาะเจ้าของร้าน','err');return;}showConfirm('ลบสินค้านี้?',()=>{DB.products=DB.products.filter(p=>p.id!==id);saveDB();showToast('ลบสินค้าแล้ว','ok');renderProducts();});}

/* ── Review ── */
let _reviewPid=null;
function openReviewModal(pid){_reviewPid=pid;const p=DB.products.find(x=>x.id===pid);const revs=DB.reviews.filter(r=>r.productId===pid);$('review-modal-body').innerHTML=`<div style="font-size:13px;font-weight:700;margin-bottom:12px"><i class="fa-solid fa-box-open"></i> ${p?.name||''}</div><div style="margin-bottom:14px">${revs.length?revs.map(r=>`<div class="comment-item"><div class="comment-meta">${r.userName} • ${'⭐'.repeat(r.rating)} • ${fmtDate(r.time)}</div><div class="comment-text">${r.text}</div></div>`).join(''):'<div style="font-size:12px;color:var(--txt-faint);text-align:center;padding:10px">ยังไม่มีรีวิว</div>'}</div><div class="form-group"><label class="form-label">คะแนน</label><div class="star-picker" id="star-picker">${[1,2,3,4,5].map(n=>`<span class="star" onclick="selectStar(${n})">☆</span>`).join('')}</div><input type="hidden" id="review-rating" value="0"></div><div class="form-group"><label class="form-label">รีวิว</label><textarea class="form-input" id="review-input" rows="3" placeholder="เขียนรีวิว..."></textarea></div>`;openModal('modal-review');}
...
function renderStockOverview(){
  ...
  $('stock-stats').innerHTML=`
    <div class="stat-card c-green"><div class="stat-icon si-green"><i class="fa-solid fa-industry"></i></div><div class="stat-label">${t('central')}</div><div class="stat-value v-green" id="ss-central">0 ชิ้น</div></div>
    <div class="stat-card c-blue"><div class="stat-icon si-blue"><i class="fa-solid fa-store"></i></div><div class="stat-label">${t('branch')}${isOwner?'รวม':u.branch}</div><div class="stat-value v-blue" id="ss-branch">0 ชิ้น</div></div>
    <div class="stat-card c-teal"><div class="stat-icon si-teal"><i class="fa-solid fa-box-open"></i></div><div class="stat-label">สินค้า</div><div class="stat-value v-teal">${DB.products.length} รายการ</div></div>
    <div class="stat-card c-red"><div class="stat-icon si-red"><i class="fa-solid fa-triangle-exclamation"></i></div><div class="stat-label">คลังต่ำ (≤10)</div><div class="stat-value v-red">${DB.products.filter(p=>p.centralStock<=10).length}</div></div>`;
  setTimeout(()=>{animateCount($('ss-central'),totC,700,'','ชิ้น');animateCount($('ss-branch'),totB,700,'','ชิ้น');},50);

  const bf=$('stock-branch-filter');
  if(bf&&isOwner){bf.style.display='flex';const brs=DB.accounts.filter(a=>a.role!=='owner');bf.innerHTML=`<span style="font-size:12px;color:var(--txt-muted);font-weight:700">${t('branch')}:</span><button class="period-tab ${!STATE._stockBranch||STATE._stockBranch==='all'?'active':''}" onclick="setStockBranch('all')">${t('allBranch')}</button>`+brs.map(b=>`<button class="period-tab ${STATE._stockBranch==b.id?'active':''}" onclick="setStockBranch(${b.id})">${b.branch}</button>`).join('');}
  ...
  $('stock-table-body').innerHTML=DB.products.map(p=>{
    ...
    return `<div class="stock-row">
      <div class="flex-row" style="gap:10px">
        ...
        <div><div class="fw-bold" style="font-size:13px">${p.name}</div><div style="font-size:10px;color:var(--txt-muted)">${p.cat}${p.costPrice>0?` • ${t('costPrice')} ฿${fmt(p.costPrice)}`:''}</div></div>
      </div>
      <div><div class="stock-num" style="color:${p.centralStock<=10?'var(--warning)':'var(--txt-primary)'}">${p.centralStock}</div><div class="stock-lbl">คลังกลาง</div></div>
      <div><div class="stock-num" style="color:${bs<=5?'var(--danger)':'var(--txt-primary)'}">${bs}</div><div class="stock-lbl">${showAll?'รวมสาขา':'ของสาขา'}</div></div>
      <div><div class="stock-num" style="color:var(--txt-muted)">${tot}</div><div class="stock-lbl">รวม</div></div>
...
function renderStockLog(){
  ...
  if(!logs.length){el.innerHTML=`<div class="empty-state"><i class="fa-solid fa-clipboard-list" style="font-size:40px;color:var(--g-200);display:block;margin-bottom:12px"></i><p>ยังไม่มีประวัติ</p></div>`;return;}
  const tl={add:'<i class="fa-solid fa-plus"></i> เพิ่ม',remove:'<i class="fa-solid fa-minus"></i> ตัด',set:'<i class="fa-solid fa-rotate"></i> กำหนด',sale:'<i class="fa-solid fa-cart-shopping"></i> ขาย',withdraw:'<i class="fa-solid fa-boxes-packing"></i> เบิก'};
  el.innerHTML=`<div class="glass-card table-wrap"><table><thead><tr><th>เวลา</th><th>สินค้า</th><th>ประเภท</th><th>จำนวน</th><th>ก่อน→หลัง</th><th>โดย</th><th>หมายเหตุ</th></tr></thead><tbody>${logs.slice(0,300).map(l=>`<tr><td style="font-size:11px;white-space:nowrap">${fmtDate(l.time)}</td><td style="font-size:12px">${l.productName}</td><td><span class="tag ${l.type==='sale'?'tag-info':l.type==='add'?'tag-success':l.type==='withdraw'?'tag-warning':'tag-danger'}">${tl[l.type]||l.type}</span></td><td class="fw-bold" style="text-align:center">${l.qty}</td><td style="font-size:12px;color:var(--txt-muted)">${l.before}→<strong>${l.after}</strong></td><td style="font-size:11px">${l.userName}</td><td style="font-size:11px;color:var(--txt-muted)">${l.note||'—'}</td></tr>`).join('')}</tbody></table></div>`;
}

/* ══════════════ LOGIN LOG ══════════════ */
function renderLoginLog(){
  const u=getCurrentUser();
  if(!u||!canManageAccounts(u.role)){$('page-loginlog').innerHTML=`<div class="denied-state"><i class="fa-solid fa-lock" style="font-size:46px;color:var(--g-200);margin-bottom:16px;display:block"></i><h3>เฉพาะเจ้าของร้าน</h3></div>`;return;}
  const el=$('loginlog-content');if(!el)return;
  if(!DB.loginLogs.length){el.innerHTML=`<div class="empty-state"><i class="fa-solid fa-lock" style="font-size:40px;color:var(--g-200);display:block;margin-bottom:12px"></i><p>ยังไม่มีประวัติ Login</p></div>`;return;}
  el.innerHTML=`<div class="glass-card table-wrap"><table><thead><tr><th>วันเวลา</th><th>ชื่อ</th><th>สาขา</th><th>บทบาท</th><th>อุปกรณ์</th></tr></thead><tbody>${DB.loginLogs.slice(0,100).map(l=>{const acc=DB.accounts.find(a=>a.id===l.userId);return`<tr><td style="font-size:11px;white-space:nowrap">${fmtDate(l.time)}</td><td style="font-size:13px;font-weight:700">${l.userName}</td><td style="font-size:12px;color:var(--txt-muted)">${l.branch}</td><td><span class="tag ${ROLE_BADGE_CLASS[acc?.role||'staff']}">${ROLE_LABEL[acc?.role||'staff']}</span></td><td style="font-size:12px">${l.device||'—'}</td></tr>`;}).join('')}</tbody></table></div>`;
}
...
function renderWithdraw(){
  ...
  if(!list.length){el.innerHTML=`<div class="empty-state"><i class="fa-solid fa-boxes-packing" style="font-size:40px;color:var(--g-200);display:block;margin-bottom:12px"></i><p>ยังไม่มีคำขอเบิก</p></div>`;return;}
  const pending=list.filter(w=>w.status==='pending'),done=list.filter(w=>w.status!=='pending');
  let html='';
  if(pending.length)html+=`<div style="font-size:11px;font-weight:800;color:var(--warning);text-transform:uppercase;letter-spacing:1px;margin-bottom:10px"><i class="fa-solid fa-hourglass-half"></i> ${t('pending')} (${pending.length})</div>`+pending.map(w=>renderWithdrawCard(w,isOwner)).join('');
  if(done.length)html+=`<div style="font-size:11px;font-weight:800;color:var(--txt-muted);text-transform:uppercase;letter-spacing:1px;margin:18px 0 10px"><i class="fa-solid fa-folder-open"></i> ดำเนินการแล้ว</div>`+done.map(w=>renderWithdrawCard(w,isOwner)).join('');
  el.innerHTML=html;updateWithdrawBadge();
}
function renderWithdrawCard(w,isOwner){
  const si=w.status==='pending'?'<i class="fa-solid fa-hourglass-half"></i>':w.status==='approved'?'<i class="fa-solid fa-circle-check"></i>':'<i class="fa-solid fa-circle-xmark"></i>';
  ...
  return `<div class="wr-card ${w.status}">
    <div class="wr-card-header">
      ...
      <div style="flex:1"><div class="wr-title">คำขอ #${w.id} — ${w.userName}</div><div class="wr-meta">${w.branch} • ${fmtDate(w.time)}</div>${w.reason?`<div class="wr-meta mt-1"><i class="fa-solid fa-pen-to-square"></i> ${w.reason}</div>`:''}</div>
      <span class="tag ${tc}">${tt}</span>
    </div>
    ${w.status==='approved'?`<div class="approved-notice"><i class="fa-solid fa-circle-check"></i> คลังกลางลดแล้ว → สต็อก${w.branch}เพิ่มแล้ว</div>`:''}
    <div class="wr-items">${w.items.map(i=>`<div class="wr-item-row"><span>${i.name}</span><span class="fw-bold">× ${i.qty}</span></div>`).join('')}</div>
    ${w.status==='pending'&&isOwner?`<div class="wr-actions"><button class="btn btn-danger btn-sm" onclick="rejectWithdraw(${w.id})"><i class="fa-solid fa-xmark"></i> ปฏิเสธ</button><button class="btn btn-primary btn-sm" onclick="previewApprove(${w.id})"><i class="fa-solid fa-check"></i> อนุมัติ</button></div>`:''}
  </div>`;
}
...
function previewApprove(wid){const u=getCurrentUser();if(!canApproveWithdraw(u?.role)){showToast('เฉพาะเจ้าของร้าน','err');return;}const w=DB.withdrawals.find(x=>x.id===wid);if(!w)return;STATE.pendingApproveId=wid;$('approve-preview-body').innerHTML=`<div style="font-size:13px;color:var(--txt-muted);margin-bottom:14px">การโอนสต็อกที่จะเกิดขึ้น:</div><div class="wr-items">${w.items.map(i=>{const p=DB.products.find(x=>x.id===i.productId);return`<div class="wr-item-row"><span>${i.name}</span><span style="display:flex;gap:12px"><span style="color:var(--danger)">คลัง −${i.qty}(${p?p.centralStock:0}→${p?Math.max(0,p.centralStock-i.qty):0})</span><span style="color:var(--g-500)">สาขา +${i.qty}</span></span></div>`;}).join('')}</div>`;openModal('modal-approve-preview');}
...
function openShiftSummary(){
  ...
  if(!active){showConfirm('เริ่มกะงานใหม่?',()=>{DB.shifts.unshift({id:getNextShiftId(),userId:u.id,userName:u.name,branch:u.branch,startTime:new Date(),endTime:null,salesIds:[],note:''});saveDB();showToast('เริ่มกะงานแล้ว','ok');});return;}
  ...
  $('shift-summary-body').innerHTML=`
    <div class="shift-info-row"><span><i class="fa-solid fa-user"></i> ${t('seller')}</span><strong>${u.name}</strong></div>
    <div class="shift-info-row"><span><i class="fa-solid fa-store"></i> ${t('branch')}</span><strong>${u.branch}</strong></div>
    <div class="shift-info-row"><span><i class="fa-regular fa-clock"></i> เริ่มกะ</span><strong>${fmtDate(active.startTime)}</strong></div>
    <div class="shift-info-row"><span><i class="fa-solid fa-receipt"></i> ${t('bills')}</span><strong>${ss.length} บิล</strong></div>
    <div class="shift-info-row"><span><i class="fa-solid fa-box-open"></i> ${t('items')}</span><strong>${fmt(items)} ชิ้น</strong></div>
    <div style="background:rgba(82,183,136,.07);border-radius:10px;padding:14px;margin:12px 0">
      <div class="shift-info-row"><span><i class="fa-solid fa-money-bill-wave"></i> ${t('cash')}</span><strong style="color:var(--g-500)">฿${fmt(cash)}</strong></div>
      <div class="shift-info-row"><span><i class="fa-solid fa-mobile-screen-button"></i> ${t('transfer')}</span><strong style="color:var(--info)">฿${fmt(trans)}</strong></div>
      <div class="shift-info-row" style="border-top:1px solid rgba(82,183,136,.2);margin-top:8px;padding-top:8px"><span style="font-weight:800"><i class="fa-solid fa-coins"></i> ยอดรวมกะ</span><strong style="font-size:20px;color:var(--g-700)">฿${fmt(total)}</strong></div>
    </div>
...
function renderCustomers(){
  ...
  if(!DB.customers.length){el.innerHTML=`<div class="empty-state"><i class="fa-solid fa-users" style="font-size:40px;color:var(--g-200);display:block;margin-bottom:12px"></i><p>ยังไม่มีลูกค้า</p><button class="btn btn-primary mt-2" onclick="openAddCustomer()"><i class="fa-solid fa-user-plus"></i> เพิ่มลูกค้า</button></div>`;return;}
  const sorted=[...DB.customers].sort((a,b)=>b.points-a.points);
  const tierColors=['#2D6A4F','#F4A300','#3A7CA5','#E03E52','#7B5EA7'];
  el.innerHTML=`<div class="glass-card">${sorted.map((c,i)=>`
    <div class="acc-row">
      <div class="acc-row-avatar" style="background:linear-gradient(135deg,${tierColors[i%5]},${tierColors[(i+2)%5]});color:#fff;font-size:16px;display:flex;align-items:center;justify-content:center">
        <i class="fa-solid ${i===0?'fa-crown':i===1?'fa-medal':i===2?'fa-award':'fa-user'}"></i>
      </div>
      <div style="flex:1">
        <div class="fw-bold" style="font-size:14px">${c.name}</div>
        <div style="font-size:11px;color:var(--txt-muted)"><i class="fa-solid fa-phone" style="font-size:9px"></i> ${c.phone||'ไม่มีเบอร์'} • มาแล้ว ${c.visitCount||0} ครั้ง${c.lastVisit?` • ${fmtDate(c.lastVisit)}`:''}</div>
      </div>
      <div style="text-align:right;margin-right:8px">
        <div style="font-family:'Prompt',sans-serif;font-size:18px;font-weight:800;color:var(--g-700)">${fmt(c.points)} แต้ม</div>
        <div style="font-size:11px;color:var(--txt-muted)">ยอดสะสม ฿${fmt(c.totalSpend||0)}</div>
      </div>
      <button class="btn btn-danger btn-xs" onclick="deleteCustomer(${c.id})"><i class="fa-solid fa-trash-can"></i></button>
    </div>`).join('')}</div>`;
}
...
function deleteCustomer(id){showConfirm('ลบลูกค้านี้?',()=>{DB.customers=DB.customers.filter(c=>c.id!==id);saveDB();showToast('ลบลูกค้าแล้ว','ok');renderCustomers();});}

function highlight(text,q){if(!q)return text;const idx=text.toLowerCase().indexOf(q.toLowerCase());if(idx===-1)return text;return text.slice(0,idx)+`<mark style="background:rgba(82,183,136,.25);color:var(--g-700);border-radius:2px;padding:0 1px">${text.slice(idx,idx+q.length)}</mark>`+text.slice(idx+q.length);}

function renderCustomerList(list,query){
  const el=$('customer-select-list'),nf=$('cust-not-found');if(!el)return;
  if(!list.length){el.innerHTML='';if(nf)nf.style.display='block';return;}
  if(nf)nf.style.display='none';
  const sorted=[...list].sort((a,b)=>{if(a.id===STATE.selectedCustomerId)return -1;if(b.id===STATE.selectedCustomerId)return 1;return b.totalSpend-a.totalSpend;});
  el.innerHTML=sorted.map(c=>{
    ...
    return`<div class="cust-select-row${isSelected?' cust-row-selected':''}" data-cust-id="${c.id}">
      <div class="cust-avatar-sm" style="background:${isSelected?'linear-gradient(135deg,var(--g-700),var(--p-400))':'linear-gradient(135deg,var(--g-700),var(--g-500))'}">
        <i class="fa-solid ${tierIcon}" style="font-size:13px;color:#fff"></i>
      </div>
      <div style="flex:1;min-width:0">
        <div class="cust-name-big">${highlight(c.name,query||'')}</div>
        <div class="cust-meta-sm"><i class="fa-solid fa-phone" style="font-size:9px"></i> ${highlight(c.phone||'ไม่มีเบอร์',query||'')} · มา ${c.visitCount||0} ครั้ง</div>
      </div>
  ...
  el.querySelectorAll('.cust-select-row').forEach(row=>{row.addEventListener('click',function(e){e.stopPropagation();const id=parseInt(this.getAttribute('data-cust-id'));if(!isNaN(id))selectCustomer(id);});});
}

function filterCustomerList(q){
  const query=(q||'').trim();
  if(!query){renderCustomerList(DB.customers,'');return;}
  const qLow=query.toLowerCase();
  const found=DB.customers.filter(c=>c.name.toLowerCase().includes(qLow)||(c.phone&&c.phone.replace(/[-\s]/g,'').includes(query.replace(/[-\s]/g,''))));
  renderCustomerList(found,query);
}

function openCustomerSelect(){
  const inp=$('cust-phone-input');if(inp)inp.value='';
  const nf=$('cust-not-found');if(nf)nf.style.display='none';
...
function selectCustomer(id){
  ...
  setTimeout(()=>{closeModal('modal-customer-select');renderCartCustomer();if(c)showToast(`<i class="fa-solid fa-user-check"></i> <strong>${c.name}</strong> · ${fmt(c.points)} แต้ม`,'ok');},120);
}

/* ══════════════ ACCOUNTS ══════════════ */
function renderAccounts(){
  const u=getCurrentUser();if(!u)return;
  if(!canManageAccounts(u.role)){$('page-accounts').innerHTML=`<div class="denied-state"><i class="fa-solid fa-lock" style="font-size:46px;color:var(--g-200);margin-bottom:16px;display:block"></i><h3>ไม่มีสิทธิ์เข้าถึง</h3><p>เฉพาะเจ้าของร้านเท่านั้น</p></div>`;return;}
  $('accounts-list').innerHTML=DB.accounts.map(a=>`
    <div class="acc-row">
      <div class="acc-row-avatar" onclick="openAvatarUpload(${a.id})" style="${a.avatarImg?`background-image:url('${a.avatarImg}');background-size:cover;background-position:center;`:`background:${a.role==='owner'?'linear-gradient(135deg,var(--g-700),var(--g-500))':a.role==='manager'?'linear-gradient(135deg,var(--g-500),var(--p-400))':'linear-gradient(135deg,var(--p-400),var(--p-300))'};display:flex;align-items:center;justify-content:center;`}">${a.avatarImg?'':`<i class="fa-solid ${a.role==='owner'?'fa-crown':a.role==='manager'?'fa-user-tie':'fa-user'}" style="font-size:16px;color:#fff"></i>`}</div>
      <div style="flex:1">
        <div class="fw-bold" style="font-size:14px;display:flex;align-items:center;gap:6px">
          ${a.name}
          ${a.online?'<span style="display:inline-flex;align-items:center;gap:3px;font-size:10px;background:rgba(82,183,136,.12);color:var(--g-500);border-radius:20px;padding:2px 7px;font-weight:700"><i class="fa-solid fa-circle" style="font-size:6px"></i> ออนไลน์</span>':'<span style="font-size:10px;color:var(--txt-faint)"><i class="fa-solid fa-circle" style="font-size:6px"></i> ออฟไลน์</span>'}
        </div>
        <div style="font-size:11px;color:var(--txt-muted)">@${a.username} • ${a.email||'—'} — ${a.branch}${a.lastSeen?` • เห็นล่าสุด ${fmtDate(a.lastSeen)}`:''}</div>
      </div>
      <span class="tag ${ROLE_BADGE_CLASS[a.role]}">${ROLE_LABEL[a.role]}</span>
      ${a.id!==u.id?`<button class="btn btn-danger btn-xs" onclick="deleteAccount(${a.id})"><i class="fa-solid fa-trash-can"></i></button>`:'<span class="tag tag-info" style="font-size:10px">คุณ</span>'}
    </div>`).join('');
}
...
function onPasswordInput(val){const hint=$('acc-pw-hint');if(!hint)return;const err=validatePassword(val);hint.textContent=err?`⚠️ ${err}`:(val.length>=6?'✅ รหัสผ่านโอเค':'');hint.style.color=err?'var(--warning)':'var(--g-500)';}
function saveAccount(){const u=getCurrentUser();if(!u)return;const name=$('acc-name').value.trim(),username=$('acc-username').value.trim(),email=$('acc-email')?.value.trim()||'',password=$('acc-password').value,role=$('acc-role').value,branch=$('acc-branch').value.trim()||'สาขา';if(!name||!username||!password){showToast('กรอกข้อมูลให้ครบ','err');return;}const pwErr=validatePassword(password);if(pwErr){showToast(pwErr,'err');return;}if(DB.accounts.find(a=>a.username===username)){showToast('ชื่อผู้ใช้นี้มีแล้ว','err');return;}const avatar=role==='owner'?'fa-crown':role==='manager'?'fa-user-tie':'fa-user';DB.accounts.push({id:getNextAccId(),name,username,email,password,role,branch,avatar,avatarImg:null,online:false,lastSeen:null});closeModal('modal-account');saveDB(['accounts']);showToast('เพิ่มบัญชีสำเร็จ','ok');renderAccounts();buildBranchStep();}
function deleteAccount(id){showConfirm('ลบบัญชีนี้?',()=>{DB.accounts=DB.accounts.filter(a=>a.id!==id);saveDB(['accounts']);showToast('ลบบัญชีแล้ว','ok');renderAccounts();buildBranchStep();});}

function openAvatarUpload(accId){
  const input=document.createElement('input');input.type='file';input.accept='image/*';
  input.onchange=e=>{const file=e.target.files[0];if(!file)return;const r=new FileReader();r.onload=ev=>{const acc=DB.accounts.find(a=>a.id===accId);if(!acc)return;acc.avatarImg=ev.target.result;saveDB(['accounts']);showToast('อัปเดตรูปโปรไฟล์แล้ว','ok');renderAccounts();buildBranchStep();if(accId===STATE._currentUserId){const avatarEl=$('user-avatar');if(avatarEl){avatarEl.style.backgroundImage=`url('${acc.avatarImg}')`;avatarEl.style.backgroundSize='cover';avatarEl.style.backgroundPosition='center';avatarEl.innerHTML='';}};};r.readAsDataURL(file);};
  input.click();
}
